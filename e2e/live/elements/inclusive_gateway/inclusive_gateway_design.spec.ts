...
test.describe('Inclusive gateway', () => {
    ...
    test('shows, edits and persists properties through XML import', async ({page}) => {
        ...
        await expectInclusiveGatewayXml(page, xml, editedId);
    });
...
async function editInclusiveGatewayProperties(page: Page): Promise<string> {
    await openGroup(page, 'general');
    await fillEntry(page, 'name', 'inclusive gateway edited');
...
async function expectInclusiveGatewayXml(page: Page, xml: string, id: string): Promise<void> {
    await expectValidBpmnXml(page, xml);
    await expectXmlElements(page, xml, `[id="${id}"]`, [{name: 'inclusiveGateway'}]);
