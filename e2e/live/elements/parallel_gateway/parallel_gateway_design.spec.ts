...
async function expectParallelGatewayXml(page: Page, xml: string, id: string): Promise<void> {
    await expectValidBpmnXml(page, xml);
    await expectXmlElements(page, xml, `[id="${id}"]`, [{name: 'parallelGateway'}]);
...
test.describe('Parallel gateway', () => {
    ...
    test('designs the element and exports its BPMN type and variant', async ({page}) => {
        ...
        await expectParallelGatewayXml(page, xml, ELEMENT_ID);
    });

    test('shows, edits and persists properties through XML import', async ({page}) => {
        const editedId = await editParallelGatewayProperties(page);
        ...
        await expectParallelGatewayXml(page, xml, editedId);
    });
...
async function editParallelGatewayProperties(page: Page): Promise<string> {
    await openGroup(page, 'general');
    await fillEntry(page, 'name', 'parallel gateway edited');
