...
test.describe('Receive task', () => {
    test.beforeEach(async ({page}) => {
        await appendTaskToNewProcess(page, ELEMENT_ID);
        await changeElement(page, 'Receive task');
    });
    ...
    test('shows, edits and persists properties through XML import', async ({page}) => {
        const editedId = await editReceiveTaskProperties(page);
...
test.describe('Receive task — sequential loop', () => {
    test.beforeEach(async ({page}) => {
        await appendTaskToNewProcess(page, ELEMENT_ID);
        await changeElement(page, 'Receive task');
...
test.describe('Receive task — standard loop', () => {
    test.beforeEach(async ({page}) => {
        await appendTaskToNewProcess(page, ELEMENT_ID);
        await changeElement(page, 'Receive task');
...
async function editReceiveTaskProperties(page: Page): Promise<string> {
    await openGroup(page, 'general');
    await fillEntry(page, 'name', 'receive task edited');
...
async function expectReceiveTaskXml(page: Page, xml: string, id: string): Promise<void> {
    await expectValidBpmnXml(page, xml);
    await expectXmlElements(page, xml, `[id="${id}"]`, [{name: 'receiveTask'}]);
