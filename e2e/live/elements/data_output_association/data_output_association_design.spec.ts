...
async function createDataOutputAssociationDiagram(page: Page): Promise<void> {
  ...
  await connectElements(page, 'Host_task', 'Data', ELEMENT_ID);
}

async function editDataOutputAssociationProperties(page: Page): Promise<string> {
...
async function expectDataOutputAssociationXml(page: Page, xml: string, id: string): Promise<void> {
  await expectValidBpmnXml(page, xml);
  await expectXmlElements(page, xml, `[id="${id}"]`, [{ name: 'dataOutputAssociation' }]);
  await expectXmlElements(page, xml, `BPMNEdge[bpmnElement="${id}"]`, [{ name: 'BPMNEdge' }]);
  await expectXmlElements(page, xml, '[id="Host_task"] > dataOutputAssociation', [{ attributes: { id } }]);
}

test.describe('Data output association', () => {
  test.beforeEach(async ({ page }) => {
    await createDataOutputAssociationDiagram(page);
  ...
  test('designs the element and exports its BPMN type and variant', async ({ page }) => {
    ...
    await expectDataOutputAssociationXml(page, xml, ELEMENT_ID);
  });

  test('shows, edits and persists properties through XML import', async ({ page }) => {
    const editedId = await editDataOutputAssociationProperties(page);
    ...
    await expectDataOutputAssociationXml(page, xml, editedId);
  });
