...
test.describe('Message flow', () => {
    test.beforeEach(async ({page}) => {
        await createMessageFlowDiagram(page);
    ...
    test('designs the element and exports its BPMN type and variant', async ({page}) => {
        ...
        await expectMessageFlowXml(page, xml, ELEMENT_ID);
    });

    test('shows, edits and persists properties through XML import', async ({page}) => {
        const editedId = await editMessageFlowProperties(page);
        ...
        await expectMessageFlowXml(page, xml, editedId);
    });
...
async function createMessageFlowDiagram(page: Page): Promise<void> {
    ...
    await createFromPalette(page, 'Create pool/participant', {x: 450, y: 530}, 'External_pool');
    await changeElement(page, 'Empty pool/participant');
    await connectElements(page, 'Host_task', 'External_pool', ELEMENT_ID);
}

async function editMessageFlowProperties(page: Page): Promise<string> {
    await openGroup(page, 'general');
    await fillEntry(page, 'name', 'message flow edited');
...
async function expectMessageFlowXml(page: Page, xml: string, id: string): Promise<void> {
    await expectValidBpmnXml(page, xml);
    await expectXmlElements(page, xml, `[id="${id}"]`, [{name: 'messageFlow'}]);
    await expectXmlElements(page, xml, `BPMNEdge[bpmnElement="${id}"]`, [{name: 'BPMNEdge'}]);
    await expectXmlElements(page, xml, `[id="${id}"]`, [{
        attributes: {
            sourceRef: 'Host_task',
            targetRef: 'External_pool'
        }
    }]);
}
